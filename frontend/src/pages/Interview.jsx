// Interview.jsx — warm radiance redesign
...
function EvalPanel({ ev }) {
  const { score, feedback, strengths, weaknesses, correctness_score, keyword_score, clarity_score, ideal_answer, teacher_explanation, structured_resources } = ev;
    ...
    <div className="card card-glow fade-in" style={{ marginTop:20, borderColor:`${color}33` }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:16 }}>
        {[
          ...
          <div key={s.label} style={{ background:"var(--bg-2)", borderRadius:10, padding:"12px", textAlign:"center", border:"1px solid var(--border)" }}>
            ...
            <div className="fade-in" style={{ marginTop:12, display:"flex", flexDirection:"column", gap:14 }}>
              {teacher_explanation && (
                <div style={{ background:"var(--bg-2)", padding:14, borderRadius:"var(--r)", border:"1px solid var(--border-2)" }}>
                  <div style={{ fontSize:10, fontWeight:700, textTransform:"uppercase", color:"var(--text-3)", marginBottom:6 }}>Teacher's Explanation</div>
                  <p style={{ fontSize:13, lineHeight:1.6, color:"var(--text-2)" }}>{teacher_explanation}</p>
                </div>
              )}
              {ideal_answer && (
                <div style={{ background:"var(--bg-2)", padding:14, borderRadius:"var(--r)", border:"1px solid var(--border-2)" }}>
                  <div style={{ fontSize:10, fontWeight:700, textTransform:"uppercase", color:"var(--green)", marginBottom:6 }}>Model Answer</div>
                  <p style={{ fontSize:13, lineHeight:1.6, color:"var(--text-2)", fontStyle:"italic" }}>"{ideal_answer}"</p>
                </div>
              )}
  ...
  return (
    <div className="fade-in" style={{ maxWidth:560, margin:"0 auto" }}>
      ...
      <div style={{ textAlign:"center", marginBottom:36 }}>
        ...
        <p className="page-sub">The AI will adapt every question to your level in real time.</p>
      </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:24 }}>
          ...
          <div style={{ background:"var(--flame-soft)", border:"1px solid rgba(255,107,53,0.2)", borderRadius:"var(--r)", padding:"12px 16px", display:"flex", gap:10, alignItems:"flex-start" }}>
            ...
            <p style={{ fontSize:13, color:"var(--text-2)", lineHeight:1.6 }}>
              AI generates <strong style={{ color:"var(--text)" }}>8 questions</strong>, adapts difficulty based on each answer, and asks follow-up questions when you need to elaborate.
            </p>
          </div>
...
export default function Interview() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await interviewAPI.submitAnswer(interviewId, question.id, answer.trim(), elapsed);
      ...
      if (res.interview_complete) {
        ...
        setTimeout(() => {
          ...
          setAnswer(""); setEvaluation(null); setSubmitting(false);
          startTimer();
        }, 3500);
      }
    } catch (err) {
    ...
    <div className="fade-in" style={{ maxWidth:720, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24, flexWrap:"wrap", gap:12 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <button className="btn btn-ghost btn-sm" onClick={async () => { if (!window.confirm("End interview early?")) return; clearInterval(timerRef.current); await interviewAPI.end(interviewId); navigate(`/report/${interviewId}`); }}>End early</button>
        </div>
      </div>
